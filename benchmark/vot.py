...
def parse_region(string):
  ...
  if len(tokens) == 4:
    ...
    return Polygon([Point(tokens[i], tokens[i + 1]) for i in range(0, len(list(tokens)), 2)])
  return None


def encode_region(region):
  if isinstance(region, Polygon):
    s =  ','.join(['{},{}'.format(p.x, p.y) for p in region.points])
  elif isinstance(region, Rectangle):
    s = '{},{},{},{}'.format(region.x, region.y, region.width, region.height)
  else:
    s = ""
  return s
...
if __name__ == "__main__":
  x = parse_region("291.827,203.603,396.382,264.289,442.173,185.397,337.618,124.711")
  print (x)
  vot = VOT('rectangle')
  vot._result = [x]
  vot.quit()
