...
def sample_exp_two_sides(lambda_):
    ...
    if (pos_or_neg % 2) == 0:
        ...
        pos_or_neg = -1
    lambda_ = lambda_ * pos_or_neg
    rand_uniform = sample_rand_uniform()
    return math.log(rand_uniform) / lambda_
...
class example_generator:
    def __init__(self, bbparam):
        self.lamda_shift = bbparam['lambda_shift_frac']
        self.lamda_scale = bbparam['lambda_scale_frac']
        self.min_scale = bbparam['min_scale']
        self.max_scale = bbparam['max_scale']

    def make_training_example_BBShift(self, bbox_prev, img_prev):
        bbox_prev = BoundingBox(bbox_prev[0], bbox_prev[1], bbox_prev[2], bbox_prev[3])
        target_pad, _, _, _ = cropPadImage(bbox_prev, img_prev)
        img_curr_ = img_prev
        bbox_curr_gt = bbox_prev

        bbox_curr_shift = BoundingBox(0, 0, 0, 0)
        bbox_curr_shift = bbox_curr_gt.shift(img_curr_, self.lamda_scale, self.lamda_shift,
                                             self.min_scale, self.max_scale, True, bbox_curr_shift)
        rand_search_region, rand_search_location, edge_spacing_x, edge_spacing_y = cropPadImage(bbox_curr_shift,
                                                                                                img_curr_)
        print (rand_search_location)
        print (edge_spacing_x, edge_spacing_y)

        bbox_gt_recentered = BoundingBox(0, 0, 0, 0)
        bbox_gt_recentered = bbox_curr_gt.recenter(rand_search_location, edge_spacing_x, edge_spacing_y,
                                                   bbox_gt_recentered)
        bbox = np.array([bbox_gt_recentered.x1, bbox_gt_recentered.y1,
                         bbox_gt_recentered.x2, bbox_gt_recentered.y2])
        return rand_search_region, target_pad, bbox
...
if __name__ == "__main__":
    ...
    for i in range(10000):
        prev_im = cv2.imread('test.jpg')
        prev_bb = np.array([115, 11.5, 247.5, 214.5])
        rand_search_region, target_pad, bbox_gt_scaled = objExampleGen.make_training_example_BBShift(prev_bb, prev_im)
    print ("1w iter costs", time.time()-s)
    ...
    ax1.imshow(prev_im)
    ax2.imshow(rand_search_region)
    #bbox_gt_scaled.unscale(rand_search_region)
    curr_bb = bbox_gt_scaled
    rect1 = patches.Rectangle((prev_bb[0], prev_bb[1]), prev_bb[2] - prev_bb[0], prev_bb[3] - prev_bb[1], linewidth=2,
                              edgecolor='r', facecolor="none")
    ax1.add_patch(rect1)
    #rect = patches.Rectangle((curr_bb.x1, curr_bb.y1), curr_bb.x2 - curr_bb.x1, curr_bb.y2 - curr_bb.y1, linewidth=2,
    #                         edgecolor='r', facecolor="none")
    #ax2.add_patch(rect)
    print(curr_bb.x1, curr_bb.y1, curr_bb.x2, curr_bb.y2)
    plt.show()
